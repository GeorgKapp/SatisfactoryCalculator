...
#pragma warning restore CS8714
		...
		foreach (var class1 in rootObjects)
		{
			...
			switch (class1.NativeClass)
			{
				...
				default:
					...
					return Result<DataContainer>.Failure($"{class1.NativeClass} not parsed");
			}
		}
        ...
        //Check item references for ammunition / weapons
        
		progress?.ReportOrThrow("Remove Generator Fuels with no energy value", token);
        RemoveGeneratorFuelsWithNoEnergy(data.Generators);
		...
		return Result<DataContainer>.Success(data);
	}

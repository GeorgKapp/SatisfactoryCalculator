...
internal class DataModelMappingService
{
            ...
            IOptions<PathOptions> pathOptions)
    {
        ...
        _pathOptions = pathOptions != null ? pathOptions.Value : throw new ArgumentNullException(nameof(pathOptions));
    }
    ...
    public async Task<DataModelMappingResult> MapConfigurationModelsAsync(IExtendedProgress<string>? progress = null, CancellationToken? token = null)
    {
        ...
        var minerResources = MapToMinorResources(modelContext.Resources.LoadAll(), itemDictionary, buildingDictionary);
        ...
        LinkCreatureVariants(modelContext.Creatures, creatureDictionary);
        ...
        var referenceDictionary = MapToEntityReferenceDictionary(itemDictionary, buildingDictionary, recipes, fuels, minerResources);
    ...
    private IEnumerable<IItem> MapToItemModels(IEnumerable<Data.Models.Implementation.Item> items) => items.Select(p => new Item.Item
    { 
    ...
    private IEquipment[] MapToEquipments(IEnumerable<Data.Models.Implementation.Equipment> equipments, IDictionary<string, IItem> itemDictionary) => equipments
        .Select(p => MapToEquipment(p, itemDictionary))
        .OrderBy(p => p.Name)
        .ToArray();

    private IEquipment MapToEquipment(Data.Models.Implementation.Equipment equipment, IDictionary<string, IItem> itemDictionary)
    {
        var item = itemDictionary[equipment.ClassName];
        
        var mappedEquipment = new Equipment.Equipment
        {
    ...
    // ReSharper disable once HeapView.ClosureAllocation
    private IConsumable[] MapToConsumables(IEnumerable<Data.Models.Implementation.Consumable> consumables, IDictionary<string, IItem> itemDictionary) => consumables
        .Select(p => MapToConsumable(p, itemDictionary))
        .OrderBy(p => p.Name)
        .ToArray();

    private IConsumable MapToConsumable(Data.Models.Implementation.Consumable consumable, IDictionary<string, IItem> itemDictionary)
    {
        var item = itemDictionary[consumable.ClassName];
        
        var mappedConsumable = new Consumable.Consumable
        {
            ClassName = item.ClassName, 
            Name = item.Name,
            Description = item.Description, 
    ...
    // ReSharper disable once HeapView.ClosureAllocation
    private IWeapon[] MapToWeapons(IEnumerable<Data.Models.Implementation.Weapon> weapons, IDictionary<string, IItem> itemDictionary) => weapons
        .Select(p => MapToWeapon(p, itemDictionary))
        .OrderBy(p => p.Name)
        .ToArray();

    private IWeapon MapToWeapon(Data.Models.Implementation.Weapon weapon, IDictionary<string, IItem> itemDictionary)
    {
        var item = itemDictionary[weapon.ClassName];
        
        var mappedWeapon = new Weapon.Weapon
        {
            ClassName = item.ClassName, 
            Name = item.Name,
            Description = item.Description, 
    ...
    // ReSharper disable once HeapView.ClosureAllocation
    private IResource[] MapToResources(IEnumerable<Data.Models.Implementation.Resource> resources, IDictionary<string, IItem> itemDictionary) => resources
        .Select(p => MapToResource(p, itemDictionary))
        .OrderBy(p => p.Name)
        .ToArray();

    private IResource MapToResource(Data.Models.Implementation.Resource resource, IDictionary<string, IItem> itemDictionary)
    {
        var item = itemDictionary[resource.ClassName];
        
        var mappedResource = new Resource.Resource
        {
            ClassName = item.ClassName, 
            Name = item.Name,
            Description = item.Description, 
    ...
    // ReSharper disable once HeapView.ClosureAllocation
    private IAmmunition[] MapToAmmunitions(IEnumerable<Data.Models.Implementation.Ammunition> ammunitions, IDictionary<string, IItem> itemDictionary) => ammunitions
        .Select(p => MapToAmmunition(p, itemDictionary))
        .OrderBy(p => p.Name)
        .ToArray();

    private IAmmunition MapToAmmunition(Data.Models.Implementation.Ammunition ammunition, IDictionary<string, IItem> itemDictionary)
    {
        var item = itemDictionary[ammunition.ClassName];
        
        var mappedAmmunition = new Ammunition.Ammunition
        {
            ClassName = item.ClassName, 
            Name = item.Name,
            Description = item.Description, 
    ...
    private void LinkWeaponsAndAmmunition(IEnumerable<Data.Models.Implementation.Weapon> weapons, IEnumerable<Data.Models.Implementation.Ammunition> ammunitions, IDictionary<string, IItem> itemDictionary)
    {
    ...
    private IEnumerable<IBuilding> MapToBuildingModels(IEnumerable<Data.Models.Implementation.Building> buildings)
    {
    ...
    private IStatue[] MapToStatueModels(IEnumerable<Data.Models.Implementation.Statue> statues) => statues
        .Select(MapToStatueModel)
        .OrderBy(p => p.Name)
        .ToArray();

    private IStatue MapToStatueModel(Data.Models.Implementation.Statue statue) =>
        new Models.Statue()
        {
    ...
    // ReSharper disable once HeapView.ClosureAllocation
    private IGenerator[] MapToGeneratorModels(IEnumerable<Data.Models.Implementation.Generator> generators, IDictionary<string, IBuilding> buildingDictionary) => generators
            .Select(p => MapToGeneratorModel(p, buildingDictionary))
            .OrderBy(p => p.Name)
            .ToArray();

    private IGenerator MapToGeneratorModel(Data.Models.Implementation.Generator generator, IDictionary<string, IBuilding> buildingDictionary)
    {
        var buildingReference = buildingDictionary[generator.ClassName];
        
        var mappedGenerator = new Generator.Generator
        {
    ...
    private IMiner[] MapToMinerModels(IEnumerable<Data.Models.Implementation.Miner> miner, IDictionary<string, IBuilding> buildingDictionary) => miner
        .Select(p => MapToMinerModel(p, buildingDictionary))
        .OrderBy(p => p.Name)
        .ToArray();

    private IMiner MapToMinerModel(Data.Models.Implementation.Miner miner, IDictionary<string, IBuilding> buildingDictionary)
    {
        var buildingReference = buildingDictionary[miner.ClassName];
        
        var mappedMiner = new Miner.Miner()
        {
    ...
    private MinerResource[] MapToMinorResources(IEnumerable<Data.Models.Implementation.Resource> resources, IDictionary<string, IItem> itemDictionary, IDictionary<string, IBuilding> buildingDictionary)
    {
    ...
    private GeneratorFuel[] MapToFuelModels(IQueryable<Data.Models.Implementation.Generator> generators, IGenerator[] generatorModels, IDictionary<string, IItem> itemDictionary)
    {
    ...
    private GeneratorFuel MapToFuelModel(FuelItem fuel, IGenerator generator, IDictionary<string, IItem> itemDictionary)
    {
        ...
        Generator.FuelItem? byProductItem = null;
    ...
    private Generator.FuelItem MapToFuelContentModel(IItem item, decimal amount) => new(item, amount);
    
    // ReSharper disable once HeapView.ClosureAllocation
    private ICreature[] MapToCreatures(IEnumerable<Data.Models.Implementation.Creature> creatues) => creatues
        .Select(MapToCreature)
        .OrderBy(p => p.Name)
        .ToArray();

    private ICreature MapToCreature(Data.Models.Implementation.Creature creatue) =>
    ...
    private void LinkCreatureVariants(IEnumerable<Data.Models.Implementation.Creature> creatures, IDictionary<string, ICreature> creatureDictionary)
    {
        var creatureArray = creatures.ToArray();
        foreach (var creature in creatureArray)
        {
            var creatureModel = creatureDictionary[creature.ClassName];
            creatureModel.Variants = MapToVariants(creatureArray, creatureModel.ClassName, creatureDictionary);
        }
    }
    
    private ICreature[] MapToVariants(IEnumerable<Data.Models.Implementation.Creature> creatures, string creatureClassName, IDictionary<string, ICreature> creatureDictionary)
    {
        return
            creatures
    ...
    private CreatureLoot[] MapToCreatureLoots(IEnumerable<Data.Models.Implementation.Creature> creatures, IDictionary<string, IItem> itemDictionary, IDictionary<string, ICreature> creatureDictionary) =>
        creatures
    ...
    private CreatureLoot MapToLoot(int amount, ICreature mappedCreature, IItem mappedItem) =>
        new()
        {
            Item = mappedItem,
            Amount = amount,
    ...
    private IRecipe[] MapToRecipeModels(IEnumerable<Data.Models.Implementation.Recipe> recipes, IDictionary<string, IItem> itemDictionary, IDictionary<string, IBuilding> buildingDictionary) =>
        recipes
            .Select(p => MapToRecipeModel(p, itemDictionary, buildingDictionary))
            .OrderRecipeModel();

    private IRecipe MapToRecipeModel(Data.Models.Implementation.Recipe recipe, IDictionary<string, IItem> itemDictionary, IDictionary<string, IBuilding> buildingDictionary)
    {
    ...
    private FactoryPlanner.FactoryConfiguration[] MapToFactoryConfigurations(IEnumerable<FactoryConfiguration> factoryConfigurations, IDictionary<string, IItem> itemDictionary, IDictionary<string, IBuilding> buildingDictionary) => factoryConfigurations
        .Select(p => MapToFactoryConfiguration(p, itemDictionary, buildingDictionary))
        .ToArray();

    private FactoryPlanner.FactoryConfiguration MapToFactoryConfiguration(FactoryConfiguration factoryConfiguration, IDictionary<string, IItem> itemDictionary, IDictionary<string, IBuilding> buildingDictionary)
    {
        var mappedfactoryConfiguration = new FactoryPlanner.FactoryConfiguration
        {
    ...
    private FactoryPlanner.FactoryConfigurationOutput MapToFactoryConfigurationOutput(FactoryConfigurationOutput factoryConfigurationOutput, IDictionary<string, IItem> itemDictionary, IDictionary<string, IBuilding> buildingDictionary)
    {
        ...
        return new()
        {
            ...
            BuildingAmount = factoryConfigurationOutput.BuildingAmount
        };
    }
    ...
    // ReSharper disable once HeapView.ClosureAllocation
    private IDictionary<string, EntityReference> MapToEntityReferenceDictionary(IDictionary<string, IItem> itemDictionary, IDictionary<string, IBuilding> buildingDictionary, IRecipe[] recipes, GeneratorFuel[] fuels, MinerResource[] minerResources)
    {
    ...
    private CreatureLoot[] GetCreatureDropsLootReferences(string entityClassName, IEnumerable<CreatureLoot> creatureLoots) => creatureLoots
        .Where(creatureLoot => creatureLoot.Creature.ClassName == entityClassName)
        .ToArray();
    
    private CreatureLoot[] GetItemIsDroppedFromCreatureReferences(string entityClassName, IEnumerable<CreatureLoot> creatureLoots) => creatureLoots
